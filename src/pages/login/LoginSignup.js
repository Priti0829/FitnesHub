...
const LoginSignup = () => {
	...
	const login = async () => {
		console.log("login")
		let signupres = await fetch('http://localhost:4000/auth/login', {
			method: "POSt",
			headers: {
	...
	const signup = async () => {
		console.log("signup")
		let signupres = await fetch('http://localhost:4000/auth/createuser', {
			method: "POSt",
			headers: {
				Accept: "application/json",
				"Content-Type": "application/json"
			...
			<div className='container'>
				<div className='d-flex justify-content-center' style={{ minHeight: "90vh" }}>
					<div className="main mt-5 mb-3">
						...
						<div className="login">
							<form className="form" onSubmit={handleSubmit} style={{ border: "none" }}>
								<label className='label' htmlFor="chk" aria-hidden="true">Log in</label>
